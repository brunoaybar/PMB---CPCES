...
// +-------------------------------------------------+
// $Id: install_rep.php,v 1.53 2021/05/04 14:46:04 dbellamy Exp $
...
if ( ($Submit == "OK") && ( ($dbname!="") || ($dbnamedbhost!="") ) ) {
	...
	if(!$use_existing_db) {
		...
		$create_pmb_user_query = "GRANT SELECT ,INSERT ,UPDATE ,DELETE ,CREATE ,DROP ,INDEX ,ALTER ,CREATE TEMPORARY TABLES ,LOCK TABLES ON ".$dbname.".* to $user@localhost identified by '$password' ";
	...
	install::createDbParam($dbhost,$usermysql,$passwordmysql,$dbname,$charset);
	...
	$update_pmb_admin_password_query = "UPDATE users SET pwd=PASSWORD('admin'), user_digest = '".md5("admin".":".md5("http://SERVER/DIRECTORY/").":"."admin")."' WHERE username='admin'";
