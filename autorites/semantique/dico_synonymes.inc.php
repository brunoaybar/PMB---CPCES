...
// +-------------------------------------------------+
// $Id: dico_synonymes.inc.php,v 1.15 2020/05/14 15:02:19 dgoron Exp $action $mot
...
switch ($action) {
	...
	case 'modif':
		...
		if ($word_selected) {
			...
			if ($word_id_selected)$rqt_ins = "update mots set mot='".$word_selected."' where id_mot='$word_id_selected' ";
			else $rqt_ins ="insert into mots set mot='".$word_selected."' ";
			
			@pmb_mysql_query($rqt_ins);
			if (!$word_id_selected)$word_id_selected= pmb_mysql_insert_id();		
		} else {
				$bool_erreur=true;
