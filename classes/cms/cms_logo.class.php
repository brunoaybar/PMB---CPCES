...
// +-------------------------------------------------+
// $Id: cms_logo.class.php,v 1.24.8.3 2022/05/16 08:26:31 tsamson Exp $
...
class cms_logo {
	...
	public function clean_cache($id = 0,$opac=false){
		...
		if(file_exists($path."/temp/cms_vign/".$database)){
			$dh = opendir($path."/temp/cms_vign/".$database);
			while($mode = readdir($dh)){
				if($mode != "." && $mode!= ".." && $mode != "CVS"){
					$mh = opendir($path."/temp/cms_vign/".$database."/".$mode);	
					while($file = readdir($mh)){
					    if($file && $file != "." && $file!= ".." && $file != "CVS" && (!$id || $file = $this->type.$id.".png") && file_exists($path."/temp/cms_vign/".$database."/".$mode."/".$file)){
						    unlink($path."/temp/cms_vign/".$database."/".$mode."/".$file);
						}
					}
					closedir($mh);
				}
