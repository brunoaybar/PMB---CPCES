...
// +-------------------------------------------------+
// $Id: list_configuration_external_services_general_ui.class.php,v 1.1.2.2 2022/04/28 12:22:25 dgoron Exp $
...
class list_configuration_external_services_general_ui extends list_configuration_external_services_ui {
	...
	public function get_display_header_list() {
		...
		$display = '<thead>';
		$display .= '<tr>';
		$display .= '<th colspan="3">Groupe</th>';
		$display .= '<th colspan="3">'.htmlentities($msg['external_services_general_utilisateurs_autorises'],ENT_QUOTES,$charset).'</th>';
		$display .= '</tr>';
		$display .= '</thead>';
		return $display;
	}
	...
	protected function get_display_group_header_list($group_label, $level=1, $uid='') {
		...
		<thead>
			<tr>
				<td></td>
				<th colspan='2'>".htmlentities($msg["external_services_general_methode"],ENT_QUOTES,$charset)."</th>
				<th colspan='3'>".htmlentities($msg["external_services_general_utilisateurs_autorises"],ENT_QUOTES,$charset)."</th>
			<tr>
		</thead>";
		return $display;
	}
