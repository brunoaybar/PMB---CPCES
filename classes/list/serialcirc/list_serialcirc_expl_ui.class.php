...
// +-------------------------------------------------+
// $Id: list_serialcirc_expl_ui.class.php,v 1.1.2.14 2022/04/05 12:13:54 dgoron Exp $
...
// 						'no_ret' => 'serialcirc_no_ret_circ',
	...
	protected function get_selection_query($type) {
		...
		switch ($type) {
			case 'locations':
				$query = 'SELECT idlocation as id, location_libelle as label FROM docs_location, docsloc_section WHERE num_location=idlocation ORDER BY label';
				break;
			case 'abts_abts':
	...
	protected function get_search_filter_location() {
		return $this->get_search_filter_simple_selection($this->get_selection_query('locations'), 'location');
	}
	...
	protected function _get_query_filters() {
		...
		if(!empty($this->filters['abts_abts'])) {
			$filters [] = 'abts_abts.abt_id IN ('.implode(',', $this->filters['abts_abts']).')';
		}
		if(!empty($this->filters['serials'])) {
			$filters [] = 'bulletin_notice IN ('.implode(',', $this->filters['serials']).')';
		}
		...
		switch($property) {
			...
			case 'actions':
				switch ($object->get_classement()) {
					...
					case 'no_ret_circ':
						$content .= "
							<input type=\"button\" class='bouton' value='".htmlentities($msg["63"],ENT_QUOTES,$charset)."' onClick=\"my_serialcirc_delete_circ('no_ret_circ','".$object->get_expl_id()."'); return false;\"/>&nbsp;
							<input type=\"button\" class='bouton' value='".htmlentities($msg["serialcirc_circ_list_bull_circulation_imprimer_bt"],ENT_QUOTES,$charset)."' onClick=\"my_serialcirc_print_list_circ('no_ret_circ','".$object->get_expl_id()."'); return false;\"/>&nbsp;
						";
						break;
				}
				break;
