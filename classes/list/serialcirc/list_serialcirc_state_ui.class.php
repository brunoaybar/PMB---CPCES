...
// +-------------------------------------------------+
// $Id: list_serialcirc_state_ui.class.php,v 1.1.2.2 2022/03/25 07:40:57 dgoron Exp $
...
class list_serialcirc_state_ui extends list_serialcirc_ui {
	...
	protected function _get_query_order() {
		if ($this->applied_sort[0]['by']) {
			...
			switch($sort_by) {
				case 'serial':
					$order .= 'notices.index_sew, abts_abts.abt_name, serialcirc_diff.serialcirc_diff_order, serialcirc_group.serialcirc_group_order';
					break;
				default :
	...
	protected function _get_query_filters() {
		...
		if($this->filters['location']) {
			$filters [] = 'abts_abts.location_id = "'.$this->filters['location'].'"';
		}
		if(!empty($this->filters['caddies'])) {
			foreach ($this->filters['caddies'] as $caddie_id) {
				...
				if (pmb_mysql_num_rows($result)) {
					...
					$filters [] = 'notices.notice_id IN ('.$notices_ids.')';
				}
			}
		}
		if(!empty($this->filters['serials'])) {
			$filters [] = 'notices.notice_id IN ('.implode(',', $this->filters['serials']).')';
		}
		if($this->filters['abts_date_fin_start']) {
			$filters [] = 'abts_abts.date_fin >= "'.$this->filters['abts_date_fin_start'].'"';
		}
		if($this->filters['abts_date_fin_end']) {
			$filters [] = 'abts_abts.date_fin < "'.$this->filters['abts_date_fin_end'].'"';
		}
		if(count($filters)) {
