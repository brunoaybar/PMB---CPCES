...
// +-------------------------------------------------+
// $Id: onto_common_datatype_list_ui.class.php,v 1.12.2.3 2022/04/05 14:23:36 tsamson Exp $
...
class onto_common_datatype_list_ui extends onto_common_datatype_ui {
	...
	protected static function get_options_values($property) {
	    ...
	    if (isset($property->pmb_list_item)) {
	        foreach ($property->pmb_list_item as $list_item) {
	            $options_values = static::set_option_value($options_values[$list_item['order']], $list_item['value'], $options_values);
	        }
	    }
	...
	private function set_option_value($order, $value, $options_values) {
	    if (isset($options_values[$order])) {
	        $order = intval($order) + 1;
	        return static::set_option_value($order, $value, $options_values);
	    }
	    $options_values[$order] = $value;
	    return $options_values;
	}
