...
// +-------------------------------------------------+
// $Id: emprunteur.class.php,v 1.260.2.18 2022/04/28 09:27:43 dgoron Exp $
...
class emprunteur {
	...
	public function do_fiche() {
		...
		if (password::check_ext_auth_file() === false) {
...
// 				$vdr=max($vdr,$valeur['org_ret_date']);
				if($forcage_prolongation== FALSE && $valeur['cpt_prolongation']>=$pret_nombre_prolongation ) {
				    ...
				    if ($pmb_utiliser_calendrier) {
				    	$onclick = "loan_extend('".$valeur['id']."', '".$deflt2docs_location."');";
				    } else {
				    	$onclick = "document.forms['prolong".$valeur['id']."'].submit();";
				    }
				    $prets_list[$valeur['short_loan_flag']] .= "
                        <input type='date' id='date_retour_".$valeur['id']."'  name='date_retour' value=\"${valeur['sql_date_retour']}\">
...
// 		return planning_list (0, 0, $this->id, '', '', LECTEUR_INFO_GESTION) ;
	...
	public static function hash_password($empr_login='',$empr_password='', $old_hash = false) {
		...
		if ($id_empr) {
		    if ($old_hash && (password::check_ext_auth_file() === false)) {
