...
// +-------------------------------------------------+
// $Id: contribution_area.class.php,v 1.42.2.3 2022/03/15 14:33:53 tsamson Exp $
...
class contribution_area {
	...
	private function get_attachment_detail($source_uri,$source_id="", $entityType = []){
		$details = array();
		$attachments = $this->get_attachment($source_uri);
		for($i=0 ; $i<count($attachments) ; $i++){
			$infos = $this->get_infos($attachments[$i]->dest);
			
			if (!in_array($infos['entityType'], $entityType)) {
			...
			$details[] = $infos;
			$details = array_merge($details, $this->get_attachment_detail('<'.$attachments[$i]->dest.'>', $infos['id'], $entityType));
