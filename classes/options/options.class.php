...
// +-------------------------------------------------+
// $Id: options.class.php,v 1.2 2021/05/10 07:54:36 dgoron Exp $
...
class options {
	...
	protected function get_label_cell_content_form($label, $name) {
		...
		switch ($name) {
			case 'MAXSIZE' :
				return $label . "<br /><span style='font-size: 0.8em'>".$msg['procs_options_text_max_help']."</span>";
			default :
				return $label;
		}
	}
		...
		switch ($field_type) {
			case 'checkbox' :
				if (empty ( $default_value ))
					$default_value = '1';
				return "<input type='checkbox' name='" . $name . "' " . ($this->parameters[$name] [0] ['value'] == $default_value ? "checked='checked'" : '') . " value='" . $default_value . "'/>";
			case 'number' :
				return "<input class='saisie-10em' type='text' name='" . $name . "' value='" . htmlentities($this->parameters[$name] [0] ['value'], ENT_QUOTES, $charset ) . "' />";
			case 'url' :
				return "<input class='saisie-40em' type='text' name='" . $name . "' value='" . htmlentities($this->parameters[$name] [0] ['value'], ENT_QUOTES, $charset ) . "' />";
			case 'textarea' :
				return "<textarea cols=50 rows=5 wrap=virtual name='" . $name . "'>" . htmlentities($this->parameters[$name] [0] ['value'], ENT_QUOTES, $charset ) . "</textarea>";
			case 'text' :
			default :
				return "<input class='saisie-40em' type='text' name='" . $name . "' value='" . htmlentities($this->parameters[$name] [0] ['value'], ENT_QUOTES, $charset ) . "' />";
		}
	...
	public function get_form() {
		...
		$form = "<form class='form-".$current_module."' name='formulaire' action='".static::get_controller_url_base()."' method='post'>
