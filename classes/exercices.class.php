...
// +-------------------------------------------------+
// $Id: exercices.class.php,v 1.25.4.3 2022/04/25 15:15:30 dgoron Exp $
...
class exercices{
	...
	public function get_form($id_entite) {
	    global $msg;
	    global $charset;
	    global $exer_content_form;
	    global $ptab;
	    
	    $content_form = $exer_content_form;
	    
	    $content_form = str_replace('!!id_entite!!', $id_entite, $content_form);
	    $content_form = str_replace('!!id_exer!!', $this->id_exercice, $content_form);
	    ...
	    if(!$this->id_exercice) {
	        ...
	        $content_form = str_replace('!!date_deb!!', $interface_date->get_display(), $content_form);
	        $interface_date = new interface_date('date_fin');
	        $content_form = str_replace('!!date_fin!!', $interface_date->get_display(), $content_form);
	        ...
	        if (exercices::hasBudgets($this->id_exercice) || exercices::hasActes($this->id_exercice)) {
	            $content_form = str_replace('!!date_deb!!', formatdate($this->date_debut), $content_form);
	            $content_form = str_replace('!!date_fin!!', formatdate($this->date_fin), $content_form);
	        } else {
	            $interface_date = new interface_date('date_deb');
	        ...
	        $content_form = str_replace('!!statut!!', htmlentities($ms,ENT_QUOTES,$charset), $content_form);
	        if ($aff_bt_def) {
	            $content_form = str_replace('<!-- case_def -->', $ptab[2], $content_form);
	        } else {
	            $content_form = str_replace('<!-- case_def -->', '', $content_form);
	        }
	    }
