...
// +-------------------------------------------------+
// $Id: z3950_notice.class.php,v 1.232.2.19 2022/06/13 13:18:13 dgoron Exp $
...
// 			$nb_expl_ignores=0;
	...
	public function update_in_database ($id_notice=0) {
		...
		$sql_ins = "update notices set
			...
			notice_is_new			='".$this->notice_is_new."',
			notice_date_is_new			='".$this->notice_date_is_new."'             
...
// 				$alink = '<a target="_blank" href="'.htmlspecialchars($doc_num["a"]).'">'.htmlspecialchars($doc_num["a"]).'</a>';
            ...
            if (!isset($this->notices_crees[$id_unimarc])) {
                ...
                if (empty($this->perio_id)) { 
                    ...
                    if(empty($titre530)) {
                        ...
                        $code = $this->perio_issn ?? "";
                        $this->perio_id = $this->create_record_serial(0, $titre530, $code);
                        if ($pmb_synchro_rdf) {
    ...
    public function create_links_articles($issues, $serials, $field, $notice) {
        ...
        if (empty($issues) && empty($serials)) {
            ...
            if (!empty($date_parution) && $date_parution != '0000-00-00') {
                $query = "update notices set date_parution='" . $date_parution . "' where notice_id='" . $this->notice_id . "'";
                pmb_mysql_query($query);
            }
    ...
    public function create_link_article_to_issue($id_article, $id_issue) 
    {
        if (!empty($this->bull_id) && $id_issue != $this->bull_id) {
            ...
            $this->delete_link_article_to_issue($id_article, $this->bull_id);
        }
...
//         }
     ...
     * @return boolean
     */
    private function delete_link_article_to_issue($id_article, $id_issue)
    {
        // On verifie que l'article est bien cree
        $query = "SELECT notice_id FROM notices WHERE notice_id = '" . addslashes($id_article) . "'";
        $res = pmb_mysql_query($query);
        if (pmb_mysql_num_rows($res)) {
            ...
            $res = pmb_mysql_query($query);
            if (pmb_mysql_num_rows($res)) {
                $query = "DELETE FROM analysis WHERE analysis_bulletin='" . addslashes($id_issue) . "' AND analysis_notice='" . addslashes($id_article) . "'";
                pmb_mysql_query($query);
                return true;
            }
        }
