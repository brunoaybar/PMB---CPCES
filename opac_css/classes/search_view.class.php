...
class search_view {
	...
	public static function get_display_simple_search_form() {
		...
		<form name='search_input' action='".($opac_autolevel2 ? static::format_url("lvl=more_results&autolevel1=1") : static::format_url("lvl=search_result"))."' method='post' onSubmit=\"if (search_input.user_query.value.length == 0) { search_input.user_query.value='*'; return true; }\">
			...
			$form .= "<div id='input-group'>";
			$form .= "
				<input type='text' name='user_query' id='user_query_lib' class='text_query' placeholder='Buscar...' value=\"" . htmlentities(stripslashes(static::$user_query),ENT_QUOTES,$charset) . "\" size='65' expand_mode='2' completion='suggestions' word_only='no'/>\n";
		
		$form .= "
				<input type='submit' id='buscar' name='ok' value='".$msg["142"]."' class='boutonrechercher'/>\n";
		if ($opac_show_help) {
			$form .= "<input type='button' id='ayuda' value='$msg[search_help]' class='bouton button_search_help' onClick='window.open(\"$base_path/help.php?whatis=simple_search\", \"search_help\", \"scrollbars=yes, toolbar=no, dependent=yes, width=400, height=400, resizable=yes\"); return false' />\n";
		}
		$form .= "</div>";
		/*
	...
	public static function get_display_search() {
		...
		//$display_search .= static::get_search_tabs();ESCONDO LAS TABS
