...
// +-------------------------------------------------+
// $Id: notice_affichage.class.php,v 1.538.2.4 2022/05/30 08:31:34 dgoron Exp $
	...
	public function genere_ajax($aj_type_aff,$header_only_origine=0){
		...
		if($opac_notices_depliable == 2){
	    	...
	    	$template.="
				...
				<div id=\"el!!id!!Child\" class=\"notice-child\" style=\"margin-bottom:6px;display:block;\" ".$this->notice_affichage_enrichment." ".($opac_allow_simili_search ? "simili_search='1'" : "").">
		    	</div>";
		}else{
			$template="$simili_search_script_all
				<div id=\"el!!id!!Parent\" class=\"notice-parent\">
					...
					if (!$opac_resa_planning) {
						...
						if(($this->notice->niveau_biblio=="m" || $this->notice->niveau_biblio=="b" || ($this->notice->niveau_biblio=="a" && $opac_show_exemplaires_analysis)) && $opac_resa && !$popup_resa) {
							...
							if ($_SESSION["user_code"] && $allow_book) {
								...
								if ($opac_max_resa==0 || $opac_max_resa>$nb_resa_encours) {
									if ($opac_resa_popup) {
										$ret .= "<a href='#' onClick=\"w=window.open('./do_resa.php?lvl=resa_planning&id_notice=".$resa_id_notice."&id_bulletin=".$resa_id_bulletin."&oresa=popup','doresa','scrollbars=yes,width=600,height=600,menubar=0,resizable=yes'); w.focus(); return false;\" id=\"bt_resa\">".$msg["bulletin_display_place_resa"]."</a>" ;
									} else {
