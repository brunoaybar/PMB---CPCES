...
// +-------------------------------------------------+
// $Id: cms_build.class.php,v 1.93.2.7 2022/03/28 13:02:26 gneveu Exp $
...
//					printr($cadres);
	...
	protected function manage_cache_cadres($todo,$build_object_name="",$content_type="",$content=""){
		...
		if($todo == "clean" && !file_exists($filepath)){
		...
		switch ($todo) {
			...
			case "insert":
				...
				cms_build::$hash_cache_cadres[] = $my_hash_cadre.$content_type;
				$requete="INSERT INTO cms_cache_cadres(cache_cadre_hash,cache_cadre_type_content,cache_cadre_content) 
					VALUES('".addslashes($my_hash_cadre)."','".addslashes($content_type)."','".addslashes($cache_cadre_content)."') 
					ON DUPLICATE KEY UPDATE cache_cadre_content='".addslashes($cache_cadre_content)."'";
