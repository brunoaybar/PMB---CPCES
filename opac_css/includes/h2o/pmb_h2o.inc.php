...
// +-------------------------------------------------+
// $Id: pmb_h2o.inc.php,v 1.57.2.14 2022/06/29 09:25:32 dgoron Exp $
...
function contributionLookup($name, $context){
    ...
    if ($contribution != $name) {
        ...
        array_walk($contribution_area_list, function($a) use (&$list_count) { $list_count += count($a); });
        array_walk($contribution_area_list_draft, function($a) use (&$list_count_draft) { $list_count_draft += count($a); });
        array_walk($contribution_area_list_done, function($a) use (&$list_done) { $list_done += count($a); });
