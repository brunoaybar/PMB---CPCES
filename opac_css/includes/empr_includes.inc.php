...
// +-------------------------------------------------+
// $Id: empr_includes.inc.php,v 1.169.2.4 2022/04/06 07:24:47 dgoron Exp $
	...
	if (! $dest) {
		...
		switch ($tab) {
			...
			default :
				...
				if ($empr_active_opac_renewal && strtotime($empr_date_expiration) >= strtotime('-'.$pmb_relance_adhesion.' days', time()) && strtotime($empr_date_expiration) <= time()) {
...
if ($opac_show_bandeaugauche == 0) {
	...
	$footer = str_replace('!!contenu_bandeau!!', '!!contenu_bandeau!!', $footer);
