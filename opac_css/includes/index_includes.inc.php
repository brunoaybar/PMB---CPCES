...
// +-------------------------------------------------+
// $Id: index_includes.inc.php,v 1.205.2.6 2022/01/29 14:19:29 gneveu Exp $
...
switch($lvl) {
	...
	case 'more_results':
	print '</div>';
	print '<div id=contenedor-padre>';
	print '<div id=contenedor-propio>';
...
//print "<div id=\"main\"><div id=\"main_header\"></div><div id=\"main_hors_footer\">";
...
if ($opac_show_bandeaugauche==0) {
	...
	if (!$_SESSION["user_code"]) {
		...
		if (!$opac_quick_access_logout || !$opac_quick_access) {
			$loginform__ .= "<a href=\"index.php?logout=1\" id=\"empr_logout_lnk\" style='color:#FFFFFF'>" . $msg["empr_logout"] . "</a>";
		}
	}
	
	// Envolver el formulario con la clase login-container
	$loginform = '<div class="login-container">' . $loginform . '</div>';
	...
	if($facette<>''){
		$footer=str_replace("!!contenu_bandeau!!","<div id=\"bandeau\">".$facette."</div>".$loginform,$footer);
	}else{
		$footer=str_replace("!!contenu_bandeau!!",$loginform,$footer);
	}		
	$footer= str_replace("!!contenu_bandeau_2!!",$opac_facette_in_bandeau_2?$lvl1.$facette:"",$footer);
} 
