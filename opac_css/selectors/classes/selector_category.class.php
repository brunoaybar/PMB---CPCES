...
// +-------------------------------------------------+
// $Id: selector_category.class.php,v 1.14.2.4 2022/04/11 14:06:51 dgoron Exp $
...
class selector_category extends selector_authorities {
	...
	protected function get_display_hierarchical_object($authority_id=0, $object_id=0) {
		...
		if ($thesaurus_mode_pmb && $caller=='notice') $nom_tesaurus='['.$category->thes->getLibelle().'] ' ;
		else $nom_tesaurus='' ;
		if ($category->is_under_tilde && !$category->voir_id && !$keep_tilde) {
			...
			$display .= "<a href='#' $java_comment onclick=\"set_parent('$caller', '$id_', '".htmlentities(addslashes($nom_tesaurus.$libelle_),ENT_QUOTES, $charset)."','$callback','".$category->thes->id_thesaurus."')\">";
			$display .= $label_display;
