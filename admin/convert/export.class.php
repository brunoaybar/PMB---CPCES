...
// +-------------------------------------------------+
// $Id: export.class.php,v 1.101.2.5 2022/01/10 16:39:34 qvarin Exp $
...
class export {
	...
	/**
	 * Retourne la liste des identifiants de notice a exporter en fonction des parametres
	 * 
	 * @param array $list_notice
	 * @param array $params
	 * @return array
	 */
	public static function get_list_notice_id($list_notice, $params = self::DEFAULT_PARAMS)
	{
	    if (empty($params)) {
	    ...
	    $list_notice_id = array();
	    $index = count($list_notice);
	    for ($i = 0; $i < $index; $i++) {
	        
	        $notice_id = $list_notice[$i];
	        if (!in_array($notice_id, $list_notice_id)) {
	            $list_notice_id[] = intval($notice_id);
	        }
	        
            $notice_relations = notice_relations_collection::get_object_instance($notice_id);
            if (empty($notice_relations)) {
                continue;
            }
	        
	        // On choisit d'exporter les notices meres
	        if ($params['mere'] && $params['notice_mere']) {
	            $relations = $notice_relations->get_parents();
	            foreach ($relations as $relation_list) {
	                foreach ($relation_list as $relation) {
	                    if (!in_array($relation->get_linked_notice(), $list_notice_id)) {
	                        $list_notice_id[] = intval($relation->get_linked_notice());
	                    }
	                }
	            }
	        }
	        ...
	        // On choisit d'exporter les notices filles
	        if ($params['fille'] && $params['notice_fille']) {
	            $relations = $notice_relations->get_childs();
	            foreach ($relations as $relation_list) {
	                foreach ($relation_list as $relation) {
	                    if (!in_array($relation->get_linked_notice(), $list_notice_id)) {
	        ...
	        // On choisit d'exporter les notices horizontales
	        if ($params['horizontale'] && $params['notice_horizontale']) {
	            $relations = $notice_relations->get_pairs();
	            foreach ($relations as $relation_list) {
	                foreach ($relation_list as $relation) {
	                    if (!in_array($relation->get_linked_notice(), $list_notice_id)) {
	        ...
	        // On choisit d'exporter les périodiques pour les notices d'article
	        if ($params["perio_link"] && $params["notice_perio"]) {
	            $query = "SELECT DISTINCT notice_id FROM bulletins, analysis, notices 
                        WHERE bulletin_notice=notice_id and bulletin_id=analysis_bulletin and analysis_notice=".$notice_id;
	            $result = pmb_mysql_query($query);
	            if (pmb_mysql_num_rows($result)) {
	                while($row = pmb_mysql_fetch_assoc($result)) {
	                    if (!in_array($row['notice_id'], $list_notice_id)) {
	                        $list_notice_id[] = intval($row['notice_id']);
	                    }
	                }
	            }
	        }
	        
	        // On choisit d'exporter les articles pour les notices de périodique
	        if ($params["art_link"] && $params["notice_art"]) {
	            $query = "SELECT DISTINCT analysis_notice AS 'notice_id' FROM notices 
                            LEFT JOIN bulletins ON bulletin_notice=notices.notice_id 
                            LEFT JOIN analysis ON analysis_bulletin=bulletin_id 
                            JOIN notices AS notices_2 ON notices_2.notice_id=analysis_notice WHERE notices.notice_id=".$notice_id;
	            $result = pmb_mysql_query($query);
	            if (pmb_mysql_num_rows($result)) {
	                while($row = pmb_mysql_fetch_assoc($result)) {
	                    if (!in_array($row['notice_id'], $list_notice_id)) {
