...
global $start, $v_state, $spec, $count, $current_module, $index_quoi;
...
switch ($index_quoi) {	
	case 'AUTHORS':
		...
		$query = pmb_mysql_query("SELECT author_id as id from authors LIMIT $start, $lot");
	...
	case 'CATEGORIES':
		if (!isset($count) || !$count) {
			$elts = pmb_mysql_query("SELECT count(distinct num_noeud) FROM categories");
			$count = pmb_mysql_result($elts, 0, 0);
			//On controle qu'il n'y a pas d'autorité à enlever
