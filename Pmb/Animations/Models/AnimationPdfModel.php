...
// +-------------------------------------------------+
// $Id: AnimationPdfModel.php,v 1.3 2020/10/05 09:12:00 gneveu Exp $
...
class AnimationPdfModel
{
    ...
    public static function renderRegistrationList(int $id)
    {
        ...
        if(file_exists($include_path.'/templates/printRegistrationList_subst.tpl.html')){
            $template_path = $include_path.'/templates/printRegistrationList_subst.tpl.html';
        }
        
        if(file_exists($template_path)){
            
            $animation = new AnimationModel($id);
            $animation->fetchPrices();
            $animation->event = $animation->getFormatDate($animation->fetchEvent());
