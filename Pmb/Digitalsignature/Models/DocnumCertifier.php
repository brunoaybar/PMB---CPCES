...
// +-------------------------------------------------+
// $Id: DocnumCertifier.php,v 1.19.2.5 2022/07/05 08:55:28 jparis Exp $
...
class DocnumCertifier implements SignInterface
{
    ...
    public function sign($idSignature): void
    {
        global $msg;
        ...
        if(isset($plugins->get_plugins()["digital_signature"])) {
            if(!empty($event_before->get_errors())) {
                error_message("pb_signature", $event_before->get_errors(), true, "./catalog.php?categ=edit_explnum&id=" . $this->entity->explnum_notice . "&explnum_id =" . $this->entity->explnum_id);die();
            }
        }
        ...
        $this->signature = new SignatureModel($idSignature);
