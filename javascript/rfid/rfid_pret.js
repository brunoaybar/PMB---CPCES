...
// +-------------------------------------------------+
// $Id: rfid_pret.js,v 1.24.2.4 2022/06/01 09:04:58 jparis Exp $
var count = 0;
...
function f_read_expl(cb,index,indexcount,antivol, cb_valid = []) {
	...
	for(var obj_cb in cb_valid){
		if(false == cb_valid[obj_cb]){
			var indication = "Il manque des &eacute;l&eacute;ments";
...
function one_more_ligne (cb_expl,data) {
	...
	//Boutton d'annulation du pret effectu� (ou pas, si erreur)
	...
	//byId('table_pret_tmp').getElementsByTagName('tbody')[0].appendChild(tr);
	document.getElementById('table_pret_tmp').appendChild(tr);
	//Si erreur on d�sactive la possibilit� d'ajout de document
	pret_erreur='';
	if(this_param[cb_expl]['status']!=0){		
...
function Ajax_add_cb(cb_doc) {	
	// R�cup�ration de la valeur de l'objet 			
...
function ack_antivol_pret(retVal) {
	...
	// suivant	
		...
		// fin des d�sactiv	ation antivol, on ferme l'iframe 
...
var mode1_liste_cb_echec_antivol=new Array();// liste des cb dont la d�sactivation de l'antivol a �chou�
...
function mode1_f_read_expl(cb,index,indexcount,antivol,uid, cb_valid = []) {
	...
	for (var i=0; i<tab_cb_nouveau.length; i++) {
		// creer nouveaux objects dans le tableau de pr�t
	...
	for(var obj_cb in cb_valid){
		if(false == cb_valid[obj_cb]){
			document.getElementById('erreur_'+obj_cb).innerHTML = "Il manque des &eacute;l&eacute;ments";
			if(document.getElementById('confirm_pret')){
				document.getElementById('confirm_pret').setAttribute("disabled", true);
				document.getElementById('confirm_pret').style = "display :none ";
			}
		} else if (cb_valid[obj_cb]) {
			document.getElementById('erreur_'+obj_cb).innerHTML = "";
			if(document.getElementById('confirm_pret')){
				document.getElementById('confirm_pret').removeAttribute("disabled");
				document.getElementById('confirm_pret').style = "display : 'inline'";
			}
		}
	}
		...
		if (is_in_array(cb, mode1_liste_cb[i])) {
			if (mode1_list_erreur_cb_count[mode1_liste_cb[i]]) {
				if(!document.getElementById('erreur_'+mode1_liste_cb[i]).innerHTML )
					document.getElementById('erreur_'+mode1_liste_cb[i]).innerHTML = "Nombre d'�l�ments manquants: "+mode1_list_erreur_cb_count[mode1_liste_cb[i]];		;
			}		
		}	
	}
	...
	// Si  liste antivol a desactiver non vide			
...
function mode1_ack_antivol_pret(retVal) {
	...
	if(!retVal ) {
		//	alert ('L\'antivol de l\'exemplaire '+mode1_tab_cb_antivol[mode1_ptr_cb_pret_liste]+' n\'a pas �t� d�sactiv� !');
	...
	if(mode1_tab_cb_antivol[++mode1_ptr_cb_pret_liste]) {
		...
		// fin des d�sactiv	ation antivol on relance les lecture
		mode1_flag_rfid_activite=0;
...
// Ajout de pr�t en saisie manuelle
...
async function mode1_confirm_pret() {
	...
	if(this.bibliotheca.isBibliotheca) {
		init_rfid_antivol("", 0, ack_antivol_pret).then(function() {
			document.location=url;
		});
	} else {
		document.location=url;
...
function mode1_one_more_ligne (cb_expl,data) {	
	...
	//oImg.setAttribute('alt', 'Antivol non d�sactiv�');
